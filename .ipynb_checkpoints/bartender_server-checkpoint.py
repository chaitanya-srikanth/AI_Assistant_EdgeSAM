...
def parse_rlef_annotations(image_annotations):
    ...
    for annotation in image_annotations:
        ...
        for vertex in annotation['vertices']:
            ...
            y = int(vertex['y'])
            bbox.append([x,y])
        labels.append(annotation['selectedOptions'][1]['value'])
        print(f" Label: {annotation['selectedOptions'][1]['value']}")
...
def make_prediction():
    ...
    try:
        ...
        bboxes, labels = parse_rlef_annotations(image_annotations)
        ...
        objects = []
        for bbox, label in zip(input_box, labels):
        ...
        # executor.submit(augment_images, image_path, objects, gzzenerated_count, Config.AUG_IMG_MODEL_ID, label)
        augment_images(filename, objects, generated_count, "663caeaddc15f50ad142073d", labels)
